...
public final class ProcessLock implements Closeable {
	...
	private static Context mContext = Easy.getContext();
    ...
    static {
        File dir = mContext.getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE);
    ...
    private static ProcessLock tryLockInternal(final String lockName, final String hash, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(
                        mContext.getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE),
                        hash);
            	...
            	HttpLog.d("tryLock: " + lockName + ", " + ignored.getMessage());
                IOUtil.closeQuietly(stream);
