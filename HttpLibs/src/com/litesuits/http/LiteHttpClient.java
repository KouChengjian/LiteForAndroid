...
/**
 * 可以由开发者自定义实现，目前默认实现为Apache的HttpClient实现 can be implemented by developer self,
 * the default implement is Apache HttpClient, now.
 *
 * @author MaTianyu 2014-1-1下午9:53:30
 */
public abstract class LiteHttpClient {
	 ...
	 * User-Agent
	 */
	public static String USER_AGENT = String.format(
			"Lite %s ( http://litesuits.com )", "1.0");
	...
	public final synchronized static LiteHttpClient newApacheHttpClient(
			Context context, String UA, boolean detectNetwork,
			boolean doStatistics, boolean forceRetry, boolean errorInChinese) {
		LiteHttpClient.USER_AGENT = UA;
		LiteHttpClient instance = ApacheHttpClient.createInstance(3000, false);
		instance.config(context, detectNetwork, doStatistics, forceRetry,
				errorInChinese);
		return instance;
	}
	...
	public abstract <T> T execute(String uri, DataParser<T> parser,
			HttpMethod method);
	...
	public abstract <T> T post(String uri, HttpParam model, HttpBody body,
			Class<T> claxx);
	 ...
	 *
	 * @param context
	 *            application 或者 activity context
	 * @param detectNetwork
	 *            连接前是否判断网络状态
	 * @param doStatistics
	 *            是否统计时间和流量
	 * @param forceRetry
	 *            非幂等请求是否强制重试
	 * @param errorInChinese
	 *            出现异常是否使用中文描述，建议国内开发者设置ture
	 */
	public void config(Context context, boolean detectNetwork,
			boolean doStatistics, boolean forceRetry, boolean errorInChinese) {
		if (context != null)
	...
	protected InternalResponse getInternalResponse(Request request) {
		...
		if (detectNetwork | doStatistics)
			innerResponse.setHttpInnerListener(new HttpInnerListener() {
				...
				public void onEnd(Response res) {
					if (doStatistics) {
						long time = start > 0 ? System.currentTimeMillis()
								- start : 0;
				...
				public void onAfterConnect(Request request) {
					if (doStatistics)
						connect += System.currentTimeMillis() - connect;
				}
				...
				public void onAfterRead(Request request) {
					if (doStatistics)
						read += System.currentTimeMillis() - read;
				}
			});
