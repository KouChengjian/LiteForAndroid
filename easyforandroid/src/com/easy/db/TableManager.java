...
import com.easy.db.annotation.Mapping;
import com.easy.db.annotation.PrimaryKey;
import com.easy.db.annotation.Table;
import com.easy.db.assit.Checker;
import com.easy.db.assit.Querier;
import com.easy.db.assit.SQLBuilder;
import com.easy.db.assit.SQLStatement;
import com.easy.db.assit.Transaction;
import com.easy.db.enums.AssignType;
import com.easy.db.model.EntityTable;
import com.easy.db.model.MapProperty;
import com.easy.db.model.Primarykey;
import com.easy.db.model.Property;
import com.easy.db.model.SQLiteColumn;
import com.easy.db.model.SQLiteTable;
...
public final class TableManager {
    ...
    private boolean checkExistAndColumns(SQLiteDatabase db, EntityTable entityTable) {
        ...
        if (sqlTable != null) {
            ...
            if (!sqlTable.isTableChecked) {
                ...
                if (EasyLog.isPrint) {
                    EasyLog.i(TAG, "Table [" + entityTable.name + "] check column now.");
                }
                if (entityTable.key != null) {
                    if (sqlTable.columns.get(entityTable.key.column) == null) {
                        ...
                        stmt.execute(db);
                        if (EasyLog.isPrint) {
                            EasyLog.i(TAG, "Table [" + entityTable.name + "] Primary Key has changed, " +
    ...
    private void putNewSqlTableIntoMap(EntityTable table) {
        if (EasyLog.isPrint) {
        	EasyLog.i(TAG, "Table [" + table.name + "] Create Success");
        }
            ...
            if (Checker.isEmpty(mSqlTableMap)) {
                ...
                Querier.doQuery(db, st, new Querier.CursorParser() {
                    ...
                    public void parseEachCursor(SQLiteDatabase db, Cursor c) throws Exception {
                        ...
                        if (EasyLog.isPrint) {
                        	EasyLog.i(TAG, "Find One SQL Table: " + sqlTable);
                        	EasyLog.i(TAG, "Table Column: " + colS);
                        }
                        mSqlTableMap.put(sqlTable.name, sqlTable);
                    }
                });
                if (EasyLog.isPrint) {
                	EasyLog.i(TAG, "Initialize SQL table end  ---------------------> " + mSqlTableMap.size());
                }
            }
        }
    }
