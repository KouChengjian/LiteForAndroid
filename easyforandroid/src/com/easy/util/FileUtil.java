...
public class FileUtil {
    ...
    public static File getCacheDir(String dirName) {
        File result;
        if (existsSdcard()) {
            File cacheDir = EasyConfig.getContext().getExternalCacheDir();
            if (cacheDir == null) {
                result = new File(Environment.getExternalStorageDirectory(),
                        "Android/data/" + EasyConfig.getContext().getPackageName() + "/cache/" + dirName);
            } else {
                result = new File(cacheDir, dirName);
            }
        } else {
            result = new File(EasyConfig.getContext().getCacheDir(), dirName);
        }
