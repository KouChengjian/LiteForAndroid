...
import com.kcj.peninkframe.utils.ShellUtil;
...
/**
 * @ClassName: SilentInstaller
 * @Description: 安装器， 静默安装、卸载（仅在root过的手机上）
 * @author:
 * @date:
...
public class SilentInstaller {
	...
	public int uninstallSilent(Context context, String packageName,
		...
		if (commandResult.responseMsg != null
				&& (commandResult.responseMsg.contains("Success") || commandResult.responseMsg
						.contains("success"))) {
			return DELETE_SUCCEEDED;
		}
		Log.e(TAG, new StringBuilder().append("uninstallSilent successMsg:")
				.append(commandResult.responseMsg).append(", ErrorMsg:")
	...
	public int getInstallLocation() {
		ShellUtil.CommandResult commandResult = ShellUtil
				.execCommand(
						...
						false, true);
		if (commandResult.result == 0 && commandResult.responseMsg != null
				&& commandResult.responseMsg.length() > 0) {
			try {
				return Integer.parseInt(commandResult.responseMsg.substring(0,
						1));
