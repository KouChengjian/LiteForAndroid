        ...
        if (retry) {
            if (appContext != null) {
                if (Network.isConnected(appContext)) {
                    ...
                    if (Log.isPrint) {
                    	Log.v(TAG, "Network is Connected Or Connecting, wait for retey : "
                                       + retrySleepTimeMS + " ms");
                    }
                ...
                if (Log.isPrint) {
                	...
                	Log.v(TAG, "wait for retry : " + retrySleepTimeMS + " ms");
                }
                Thread.sleep(retrySleepTimeMS);
            }
        }
        if (Log.isPrint) {
        	Log.i(TAG, "retry: " + retry + " , retryCount: " + retryCount + " , exception: " + exception);
        }
        return retry;
    }
