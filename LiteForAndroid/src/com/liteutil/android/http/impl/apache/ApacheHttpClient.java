...
import com.liteutil.android.http.data.Charsets;
import com.liteutil.android.http.data.Consts;
import com.liteutil.android.http.data.HttpStatus;
import com.liteutil.android.http.exception.ClientException;
import com.liteutil.android.http.exception.HttpClientException;
import com.liteutil.android.http.exception.HttpNetException;
import com.liteutil.android.http.exception.HttpServerException;
import com.liteutil.android.http.exception.ServerException;
...
import com.liteutil.android.http.response.InternalResponse;
...
//    }
    ...
    private BasicHttpParams createHttpParams() {
        ...
        ConnManagerParams.setTimeout(params, config.getConnectTimeout()); // 链接超时
        ConnManagerParams.setMaxConnectionsPerRoute(params, new ConnPerRouteBean(DEFAULT_MAX_CONN_PER_ROUT)); // 设置每个路由最大连接数 
        ...
        HttpConnectionParams.setConnectionTimeout(params, config.getConnectTimeout()); // 设置超时时间 
        HttpConnectionParams.setSoTimeout(params, config.getSocketTimeout()); // 设置读取超时时间 
        HttpConnectionParams.setSocketBufferSize(params, config.getSocketBufferSize());
    ...
    protected <T> void connectWithRetries(AbstractRequest<T> request, InternalResponse response)
        ...
        while (retry) {
            try {
                ...
                if (headers != null) {
                	com.liteutil.android.http.data.NameValuePair hs[] = new com.liteutil.android.http.data.NameValuePair[headers.length];
                    for (int i = 0; i < headers.length; i++) {
                        ...
                        hs[i] = new com.liteutil.android.http.data.NameValuePair(name, value);
                    }
            ...
            if (cause != null) {
                ...
                if (retry) {
                    ...
                    if (Log.isPrint) {
                        Log.i(TAG, "LiteHttp retry request: " + request.getUri());
                    }
                    if (listener != null) {
            ...
            try {
                ...
                if (f != null) {
                    ...
                    if (wrapped != null) {
                        ...
                        if (Log.isPrint) {
                            Log.d(TAG, "HttpEntity wrappedEntity reflection consumeContent");
                        }
                    }
                }
            } catch (Throwable t) {
                Log.e(TAG, "wrappedEntity consume error. ", t);
            }
        }
    }
}
