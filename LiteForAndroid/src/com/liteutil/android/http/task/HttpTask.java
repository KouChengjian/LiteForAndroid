...
import com.liteutil.android.http.impl.UriRequestFactory;
import com.liteutil.android.http.listener.Callback;
import com.liteutil.android.http.listener.InterceptRequestListener;
import com.liteutil.android.http.listener.RedirectHandler;
import com.liteutil.android.http.request.RequestTracker;
import com.liteutil.android.http.request.RequestTrackerWrapper;
...
import com.liteutil.android.util.ParameterizedTypeUtil;

/**
 * @ClassName: HttpTask<ResultType>
 * @Description: http 请求任务
 * @author:
 * @date:
 */
public class HttpTask<ResultType> extends AbsTask<ResultType> implements ProgressHandler {
    ...
    public ResultType doBackground() throws Throwable {
        ...
        if (cacheCallback != null && HttpMethod.permitsCache(params.getMethod())) {
            ...
            try {
                clearRawResult();
                Log.d("load cache: " + this.request.getRequestUri());
                rawResult = this.request.loadResultFromCache();
            } catch (Throwable ex) {
            	Log.w("load disk cache error", ex);
            }
            ...
            if (rawResult != null) {
                if (prepareCallback != null) {
                    try {
                        ...
                        cacheResult = null;
                        Log.w("prepare disk cache error", ex);
                    } finally {
                        clearRawResult();
                    }
                } else {
        ...
        while (retry) {
            ...
            try {
                ...
                try {
                    clearRawResult();
                    // 开始请求工作
                    Log.d("load: " + this.request.getRequestUri());
    ...
    private void closeRequest() {
        LiteHttp.task().run(new Runnable() {
            @Override
            public void run() {
                clearRawResult();
                        ...
                        if (redirectHandler != null) {
                            try {
                                ...
                                if (redirectParams != null) {
                                    ...
                                    this.result = LiteHttp.task().startSync(task);
                                    this.ex = null;
                                }
