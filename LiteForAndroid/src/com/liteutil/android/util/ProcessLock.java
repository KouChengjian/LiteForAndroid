package com.liteutil.android.util;
...
public final class ProcessLock implements Closeable {
    ...
    static {
        IOUtil.deleteFileOrDir(mContext.getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE));
    }
    ...
    public static ProcessLock tryLock(final String lockName, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(mContext.getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE), customHash(lockName));
    ...
    private static void release(String lockName, FileLock fileLock, File file, Closeable stream) {
        synchronized (LOCK_MAP) {
        	Log.d("release: " + file.getName() + ":" + PID);
            if (fileLock != null) {
