...
public class SampleAsyncActivity extends Activity implements OnItemClickListener {
	...
	private List<String> getData() {
		...
		data.add("0. AsyncTask Execute"); // 通用
		data.add("1. AsyncTask Exe Runnable"); // 
		data.add("2. Simple Task");
		data.add("3. Safe Task");
		data.add("4. Task Cancel");
		data.add("5. Ordered Task Executor");
		data.add("6. CyclicBarrier Task Executor");
		data.add("7. Delayed Task");
		data.add("8. Timer Task");
		data.add("9. Cached Task:Reduce Pressure of Server");
		return data;
	}
	...
	private void testAsyncTask() {
		...
		for (int i = 0; i < 66; i++) {
			...
			task = new AsyncTask<Void, Void, Void>() {
				@Override
				protected Void doInBackground(Void... params) {
					Log.e("TAG","AsyncTask executeAllowingLoss =====" + j);
	...
	private void testExeRunnable() {
		...
		for (int i = 0; i < 66; i++) {
			...
			AsyncTask.executeAllowingLoss(new Runnable() {
                @Override
                public void run() {
                    Log.e("TAG","AsyncTask Runnable executeAllowingLoss " + j);
                    SystemClock.sleep(500);
                }
            });
		}
	...
	private void testSimpleTask() {
		SimpleTask<Integer> simple = new SimpleTask<Integer>() {
			...
			protected void onPostExecute(Integer result) {
				Log.i("TAG","SimpleTask result: " + result);
			}
		};
		simple.execute();
		// simple safe success
